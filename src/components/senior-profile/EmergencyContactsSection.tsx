  ...
  const handleSave = async (c: ContactData) => {
    ...
    if (existing) {
      ...
      const { error } = await supabase.from("emergency_contacts").insert({
        senior_id: seniorId, name: c.name, relationship: c.relationship,
        phone: c.phone || "", email: c.email, notify_via_sms: c.notifyViaSms,
        notify_via_email: c.notifyViaEmail, sort_order: sortOrder,
        delay_minutes: autoDelays[sortOrder] ?? 60, user_id: user.id,
      });
      if (error) { toast({ title: "Could not save contact — please try again.", variant: "destructive" }); return; }
    ...
    <div className="bg-card rounded-2xl border border-border shadow-card p-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-muted transition-colors" aria-label="Help"><HelpCircle className="w-4 h-4 text-muted-foreground" /></button>
            </PopoverTrigger>
            <PopoverContent className="max-w-[320px] text-sm leading-relaxed">
              <p>If {seniorName} misses a check-in, Contact 1 is notified immediately. If they don't respond within 30 minutes, Contact 2 is notified — and so on.</p>
              <div className="flex justify-end mt-3"><Button size="sm" className="rounded-xl font-bold" onClick={() => document.dispatchEvent(new Event("close-popover"))}>Got it</Button></div>
            </PopoverContent>
          </Popover>
        </div>
      </div>
      ...
      {sorted.length === 0 ? (
        <div className="flex flex-col items-center py-8 text-center gap-3">
          ...
          <Button onClick={() => { setEditingContact(null); setModalOpen(true); }} className="rounded-xl font-black gap-1.5 mt-1"><Plus className="w-4 h-4" /> Add First Contact</Button>
        </div>
      ) : (
        <div className="space-y-3">
          {sorted.map((contact, i) => (
            <div key={contact.id} className="bg-background rounded-xl border border-border p-4">
              <div className="flex items-start gap-3">
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-black shrink-0" style={{ background: priorityColors[Math.min(i, 2)], color: "#fff" }}>{i + 1}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="font-black text-sm">{contact.name}</p>
                        {i === 0 && (<span className="px-2 py-0.5 rounded-full text-[10px] font-black" style={{ background: "hsl(var(--status-checked) / 0.12)", color: "hsl(var(--status-checked))" }}>PRIMARY</span>)}
                      </div>
                      {contact.relationship && <p className="text-xs text-muted-foreground">{contact.relationship}</p>}
                    </div>
                    <div className="flex gap-1">
                      <Button variant="ghost" size="sm" className="h-8 text-xs font-bold" onClick={() => { setEditingContact(contact); setModalOpen(true); }}><Pencil className="w-3.5 h-3.5 mr-1" /> Edit</Button>
                      <Button variant="ghost" size="sm" className="h-8 text-xs font-bold text-destructive hover:text-destructive" onClick={() => setConfirmRemoveId(contact.id)} disabled={sorted.length === 1}><Trash2 className="w-3.5 h-3.5 mr-1" /> Remove</Button>
                    </div>
                  </div>
                  {contact.phone && (<span className="inline-flex items-center gap-1 text-xs text-muted-foreground mt-1.5"><Phone className="w-3.5 h-3.5" /> {contact.phone}</span>)}
                  {contact.email && (<span className="inline-flex items-center gap-1 text-xs text-muted-foreground mt-1 ml-3"><Mail className="w-3.5 h-3.5" /> {contact.email}</span>)}
                  <div className="flex items-center gap-2 mt-2 flex-wrap">
                    {contact.notifyViaSms && (<span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold" style={{ background: "hsl(var(--primary) / 0.12)", color: "hsl(var(--primary))" }}><MessageSquare className="w-3 h-3" /> SMS ✓</span>)}
                    {contact.notifyViaEmail && (<span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold" style={{ background: "hsl(var(--primary) / 0.12)", color: "hsl(var(--primary))" }}><Mail className="w-3 h-3" /> Email ✓</span>)}
                  </div>
          ...
          {sorted.length < 3 && (
            <button onClick={() => { setEditingContact(null); setModalOpen(true); }} className="w-full py-3 rounded-xl border-2 border-dashed border-border text-sm font-bold text-muted-foreground hover:border-primary hover:text-primary transition-colors flex items-center justify-center gap-1.5"><Plus className="w-4 h-4" /> Add Emergency Contact</button>
          )}
        </div>
      )}
      <AddEditContactModal open={modalOpen} onClose={() => { setModalOpen(false); setEditingContact(null); }} contact={editingContact} onSave={handleSave} isFirstContact={isFirstContact} isMobile={isMobile} />
    </div>
  );
}
